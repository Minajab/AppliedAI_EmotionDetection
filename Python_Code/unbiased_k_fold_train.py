...
def main(args):
    ...
    all_indices = list(images_metadata_df['Idx'].values)
    category_class_combinations = list(images_metadata_df['MixedCol'].values)

    category_class_combinations_mapping = {val: category_class_combinations[idx] for idx, val in enumerate(all_indices)}

    # Get the labels of the dataset
    all_labels = [dataset[i][1] for i in all_indices]

    all_labels_mapping = {val: all_labels[idx] for idx, val in enumerate(all_indices)}
    ...
    for fold, (train_index, test_index) in enumerate(k_fold.split(all_indices, category_class_combinations), start=1):
        print(f'Fold {fold}:')
        indices = all_indices[train_index]
        labels = all_labels[train_index]
        category_combination = [category_class_combinations_mapping[i] for i in list(indices)]

        # Use train_test_split to split the dataset into train and validation sets
        train_indices, val_indices = train_test_split(indices, test_size=0.19, stratify=category_combination, random_state=42)
        ...
        with tqdm(range(num_epochs), unit='epoch') as tepoch:
            for epoch in tepoch:
                ...
                if epoch >= 9 and (val_loss / val_total_data) < best_validation_error:
                    ...
                    if not os.path.exists(os.path.join(model_path, f'unbiased_saved_models_{num_folds}_fold')):
                        os.mkdir(os.path.join(model_path, f'unbiased_saved_models_{num_folds}_fold'))
                    model_path = os.path.join(model_path, f'unbiased_saved_models_{num_folds}_fold')
                    layers_str = '_'.join(map(str, layers))
                    model_name = f'best_model_kernel_{conv_kernel_size}_pooling_kernel_{pooling_kernel_size}_{layers_str}_fold_{fold}'
        ...
        model_path = os.path.join(model_path, f'unbiased_saved_models_{num_folds}_fold')
        layers_str = '_'.join(map(str, layers))
        model_name = f'final_model_kernel_{conv_kernel_size}_pooling_kernel_{pooling_kernel_size}_{layers_str}_fold_{fold}'
        torch.save(model.state_dict(), os.path.join(model_path, f'{model_name}.pth'))
        losses_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..'))
        if not os.path.exists(os.path.join(losses_path, f'unbiased_losses_{num_folds}_fold')):
            os.mkdir(os.path.join(losses_path, f'unbiased_losses_{num_folds}_fold'))
        losses_path = os.path.join(losses_path, f'unbiased_losses_{num_folds}_fold')
        layers_str = '_'.join(map(str, layers))
        ...
        # Test the trained model
        layers_str = '_'.join(map(str, layers))
        best_model_name = f'best_model_kernel_{conv_kernel_size}_pooling_kernel_{pooling_kernel_size}_{layers_str}_fold_{fold}'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='K-Fold Cross Validation: Train an Unbiased Emotion Detection Model.')
