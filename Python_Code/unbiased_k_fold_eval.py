...
def main(args):
    ...
    if not os.path.exists(os.path.join(results_path, f'unbiased_results_{num_folds}_fold')):
        os.mkdir(os.path.join(results_path, f'unbiased_results_{num_folds}_fold'))
    results_path = os.path.join(os.path.join(results_path, f'unbiased_results_{num_folds}_fold'))

    layers_str = '_'.join(map(str, layers))
    model_name = f'best_model_kernel_{conv_kernel_size}_pooling_kernel_{pooling_kernel_size}_{layers_str}_fold_1'
    model_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '..', f'unbiased_saved_models_{num_folds}_fold'))
    ...
    for fold, (train_index, test_index) in enumerate(k_fold.split(all_indices, category_class_combinations), start=1):
        ...
        losses_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', f'unbiased_losses_{num_folds}_fold'))
        layers_str = '_'.join(map(str, layers))
