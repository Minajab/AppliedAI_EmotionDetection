...
def main(args):
    ...
    for fold, (train_index, test_index) in enumerate(k_fold.split(all_indices, all_labels), start=1):
        ...
        with tqdm(range(num_epochs), unit='epoch') as tepoch:
            for epoch in tepoch:
                ...
                if epoch >= 9 and (val_loss / val_total_data) < best_validation_error:
                    ...
                    if not os.path.exists(os.path.join(model_path, f'saved_models_{num_folds}_fold')):
                        os.mkdir(os.path.join(model_path, f'saved_models_{num_folds}_fold'))
                    model_path = os.path.join(model_path, f'saved_models_{num_folds}_fold')
        ...
        losses_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..'))
        if not os.path.exists(os.path.join(losses_path, f'losses_{num_folds}_fold')):
            os.mkdir(os.path.join(losses_path, f'losses_{num_folds}_fold'))
        losses_path = os.path.join(losses_path, f'losses_{num_folds}_fold')
        layers_str = '_'.join(map(str, layers))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='K-Fold Cross Validation: Train an Emotion Detection Model.')
